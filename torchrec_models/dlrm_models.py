...
class DLRM_MLP(nn.Module):
    ...
    def forward(
        ...
        """
        Args:
            dense_inputs (torch.Tensor): the dense inputs.
            sparse_features (torch.Tensor): the sparse features.
        Returns:
            torch.Tensor: logits.
        """
        embedded_dense = self.dense_arch(dense_inputs)
        concatenated_dense = self.inter_arch(
            dense_features=embedded_dense, sparse_features=sparse_features
        )
        logits = self.over_arch(concatenated_dense).squeeze(-1)
        return logits


class DLRM_EMB(nn.Module):
    """
    Only Contain Embedding Layer
    """
    ...
    def forward(
        ...
        # embedded_dense = self.dense_arch(dense_features)
        embedded_sparse = self.sparse_arch(sparse_features)
        batch_size, nTable, dim = embedded_sparse.shape
        logits = embedded_sparse.view(batch_size, nTable * dim).mean(dim=1)
