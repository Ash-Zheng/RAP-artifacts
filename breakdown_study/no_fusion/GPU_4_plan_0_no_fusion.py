...
def train_and_preprocessing_process(rank, nDev, nProcess, args, queue_list, input_queue_list, if_train) -> None:
    ...
    if if_train:
        ...
        sparse_name = []
        local_dense_name = []
        for name in local_sparse_list[this_rank]:
            if name[0] == 'c':
                sparse_name.append(name)
            else:
                local_dense_name.append(name)
        dense_name = args.int_name

        border = torch.tensor([1,2,3]).to(this_rank)

        data_dir = "/workspace/RAP/breakdown_study/no_fusion/generated_data/"
        ...
        int_ptr = [df_dense[d_name].data.ptr for d_name in dense_name] + [df_dense[label_name].data.ptr]
        cat_ptr = [df_sparse[s_name].data.ptr for s_name in sparse_name]
        ...
        for i in range(len(sparse_name)):
            sparse_tensor_list.append(cuda_preprocess.fill_null_int64(cat_ptr[i], args.batch_size * nDev))
        ...
        # bucketize:
        if this_rank == 0:
            dense_0 = cuda_preprocess.fill_null_float(df_sparse["int_0"].data.ptr, args.batch_size * nDev)
        ...
        print(rank, new_dist_input.shape, dense_input.shape, label_tensor.shape)
    ...
    for i in loop_range:
        ...
        if if_train:
            ...
            dist_input[0]._values = new_dist_input.clone()
            training_iter(sparse_optimizer, dense_optimizer, loss_fn, dist_input, dense_input, label_tensor, sharded_emts, mlp_layers, rank, train_group)
...
def training_iter(sparse_optimizer, dense_optimizer, loss_fn, dist_input, dense_input, label_tensor, sharded_emts, mlp_layers, rank, work_group):
