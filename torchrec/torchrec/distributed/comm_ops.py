...
class ReduceScatterBase_Req(Function):
    ...
    def forward(
        ...
        with record_function("## reduce_scatter_base ##"):
            req = dist.reduce_scatter_tensor(output, inputs, group=pg, async_op=True)
...
class AllGatherBase_Req(Function):
    ...
    def forward(
        ...
        with record_function("## all_gather_base ##"):
            req = dist.all_gather_into_tensor(outputs, input, group=pg, async_op=True)
...
class ReduceScatterV_Req(Function):
    ...
    def forward(
        ...
        if rsi.equal_splits:
            with record_function("## reduce_scatter_base ##"):
                req = dist.reduce_scatter_tensor(output, input, group=pg, async_op=True)
