...
class ShardedModule(
    ...
    # def input_comm(self, *input, **kwargs) -> Awaitable[Awaitable[CompIn]]:
    #     """
    #     Executes the input dist, return the input for the compute step.
    #     Args:
    #         *input: input.
    #         **kwargs: keyword arguments.
    #     """
    ...
    def input_comm(self, input) -> Awaitable[Awaitable[CompIn]]:
        """
        Executes the input dist, return the input for the compute step.
        Args:
            *input: input.
        ...
        dist_input = self.input_dist(self.ctx, input).wait().wait()
        return dist_input

    def forward_without_input_comm(self, dist_input) -> LazyAwaitable[Out]:
        """
        Executes the compute, and output dist steps.
        Args:
            input: dist_input.
        """
        if self.ctx is None:
            self.ctx = self.create_context()
        return self.compute_and_output_dist(self.ctx, dist_input)
