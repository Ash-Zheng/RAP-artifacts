...
parser.add_argument(
    "--sharding_plan",
    type=str,
    default="2,2,0,2,0,1,0,2,2,0,1,0,2,2,2,0,2,2,2,3,0,1,2,1,2,1",
    help="tablewise_sharding_plan",
)
...
def train_process(rank, nDev, args) -> None:
    ...
    # if rank == 0:
        # print(plan.plan['model.sparse_arch.embedding_bag_collection']['t_cat_0'])

    # generated by Gurobi Solver
    table_map = list(
        map(int, args.sharding_plan.split(","))
    )

    for i in range(len(table_map)):
        table_name = f"t_cat_{i}"
        tar_GPU = table_map[i]
        table_length = args.num_embeddings_per_feature[i]
        placement_str = "rank:{}/cuda:{}".format(tar_GPU, tar_GPU)
        plan.plan['model.sparse_arch.embedding_bag_collection'][table_name] = ParameterSharding(sharding_type='table_wise', compute_kernel='fused', ranks=[tar_GPU], sharding_spec=EnumerableShardingSpec(shards=[ShardMetadata(shard_offsets=[0, 0], shard_sizes=[table_length, args.embedding_dim], placement=placement_str)]))
    ...
    if rank == 0:
        for i in range(nIters):
            with nvtx.annotate("iter-{}".format(i)):
                batch = next(input_iter)
                if i % nRandom == nRandom-1:
                    input_iter = iter(input_data)
