...
def logit(col: NumericalColumn, eps=1e-5):
    """
    Returns the logit values of the input column

    Parameters
    ----------
    col: Numeric column that has a list of dense values
    eps: Small value to add to the input values to avoid log(0)

    """
    return _dispatch("logit", col, eps)


def boxcox(col: NumericalColumn, _lambda=1e-5):
    """
    Returns the boxcox values of the input column

    Parameters
    ----------
    col: Numeric column that has a list of dense values
    _lambda: numeric value indicating what powers to use for the Box-Cox transformation.
    """
    return _dispatch("boxcox", col, _lambda)


def onehot(col: ListColumn, lower:float, upper:float, num_class:int):
    """
    Returns the onehot of the input dence column

    Parameters
    ----------
    col: Numeric column that has a list of dense values
    lower: lower bound of the input values
    upper: upper bound of the input values
    num_class: number of classes to use for the onehot encoding
    """
    return _dispatch("onehot", col, lower, upper, num_class)


def ngram(col: ListColumn, n_of_gram:int):
    """
    Returns the an n-gram between a list of sparse features

    Parameters
    ----------
    col: Numeric column that has a list of sparse features
    n_of_gram: number of features to use for the n-gram
    """
    return _dispatch("ngram", col, n_of_gram)


def mapid(col: ListColumn, _map:ListColumn):
    """
    Returns the new sparse features after mapping the input sparse features

    Parameters
    ----------
    col: Numeric column that has a list of sparse features
    _map: map from old sparse features to new sparse features
    """
    return _dispatch("mapid", col, _map)
